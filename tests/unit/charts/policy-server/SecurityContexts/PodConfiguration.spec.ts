...
describe('component: PodConfiguration', () => {
  it('displays PodConfiguration correctly and should emit the correct events for data update upstream', async() => {
    ...
    const wrapper = shallowMount(PodConfiguration as unknown as ExtendedVue<Vue, {}, {}, {}, DefaultProps>, {
    ...
    expect(wrapper.emitted()['update-pod-config']?.length).toBe(2);
    expect(wrapper.emitted()['update-pod-config']?.[0][0]).toEqual(expect.objectContaining({ runAsUser: 1 }));
    expect(wrapper.emitted()['update-pod-config']?.[1][0]).toEqual(expect.objectContaining({ capabilities: { prop: 'some-prop' } }));
  });
});
